...
| **Asset Type** | Customer Event Log |
| **Control Record** | Customer Event Log Maintenance Agreement |
...
| POST | `/v1/customer-event-log-maintenance-agreement/initiate` | Initiate |
| GET | `/v1/customer-event-log-maintenance-agreement` | Retrieve (list) |
| GET | `/v1/customer-event-log-maintenance-agreement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/customer-event-log-maintenance-agreement/{crId}/update` | Update |
| PUT | `/v1/customer-event-log-maintenance-agreement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/customer-event-log-maintenance-agreement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-customer-event-history:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-customer-event-history ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-customer-event-history` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
